...
const Admin = () => {
  ...
  let addPost = async(e) => {
    e.preventDefault();
    let resp = await fetch("https://recruitsys.onrender.com/addPost?"+new URLSearchParams({ jwt: Jwt }), {
            method: 'POST',
            headers:{
    ...
    if(resp.status === 200) {
      const resp = await fetch(
        "https://recruitsys.onrender.com/getPosts?"+new URLSearchParams({ jwt: Jwt }),
        {
          method: "GET",
        }
      );
  ...
  let delPost = async(e) => {
    e.preventDefault();
    let resp = await fetch("https://recruitsys.onrender.com/deletePost?"+new URLSearchParams({ jwt: Jwt }), {
            method: 'POST',
            headers:{
                'Content-Type' : 'application/json',
            },
